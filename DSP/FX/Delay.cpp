...
void Delay::init(DaisySeed* seed, DelayLine<float, MAXDELAY>* dl[2])
{
    ...
    size.init(seed, 0, 10000, LINEAR, ChannelIDs::ENCODER, [this] (size_t s) { setDelay(s); }); // to be set to encoder
    bounce.param.init(seed, 0, 1, LINEAR, ChannelIDs::ENCODER, [this] (float b) { setBounce(b); }); // to be set to encoder
