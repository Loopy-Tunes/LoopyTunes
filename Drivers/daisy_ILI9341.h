#ifndef DAISY_ILI9341_H
#define DAISY_ILI9341_H
  ...
  public:
    ...
    /*void ClearBuffer(uint16_t color) { 
        for (size_t i = 0; i < buffer_size / 2; i++) {
            ...
            frame_buffer[i * 2 + 1] = color & 0xFF; // new function
        }
    }*/
    ...
    uint32_t GetTransferSize() const
    {
        return remaining_buff < buf_chunk_size ? remaining_buff
                                               : buf_chunk_size;
        // return remaining_buff < 2 * buf_chunk_size ? remaining_buff // uncommented these two lines
          //                                         : 2 * buf_chunk_size;
    }
    ...
    static uint32_t const buffer_size    = 153600;          // 320 * 240 * 2   = 153600
...
class UiDriver
{
  public:
    ...
    void Init()
    {
        screen_update_period_ = 34; // 17 is roughly 60Hz
