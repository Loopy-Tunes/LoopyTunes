...
void MixerView::init(DaisySeed* seed, EncoderDriver* encoder, UiDriver* uid, KeypadDriver* kpd)
{
    ...
    amp1.init(0, seed, uid);
    amp2.init(1, seed, uid);
    amp3.init(2, seed, uid);
    amp4.init(3, seed, uid);

    selectedChannel = 0;

    track1.init(1, encoder, lcd, keypad);
    track2.init(2, encoder, lcd, keypad);
    track3.init(3, encoder, lcd, keypad);
    track4.init(4, encoder, lcd, keypad);
